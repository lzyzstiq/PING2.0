...
public class ParticleRenderer implements GLSurfaceView.Renderer {
    ...
    // 动画状态枚举
    private enum State {
        CONVERGE, WRITING, FLASH
    }

    // 核心参数
    private static final int PARTICLE_COUNT = 1500;
    private static final float CONVERGE_DURATION = 4.0f;          // 汇聚阶段持续时间（秒）
    private static final float PER_CHAR_TIME = 2.0f;              // 每个字符书写时间（秒）
    private static final float MAX_WRITING_TIME = 8.0f;           // 最大书写时间（秒）
    private static final float GRAVITY_STRENGTH = 1000.0f;        // 引力常数
    private static final float ABSORB_RADIUS = 10.0f;             // 黑洞吸收半径
    private static final float PARTICLE_BASE_SIZE = 8.0f;         // 粒子基础大小
    private static final float[] PARTICLE_COLOR = {0.9f, 0.72f, 0.0f, 1.0f}; // 暖金色 RGBA
    private static final float FLASH_PARTICLE_COUNT_PER_CHAR = 20; // 每个字符的闪烁粒子数

    private Context context;
    private String quoteText = "子瑜，晚安";  // 默认语录
    private AnimationListener listener;
    private boolean isRunning = true;
    private boolean isFlashing = true;       // 闪烁状态是否激活
    ...
    // 粒子数据 (交错布局: x, y, vx, vy, life)
    private static final int STRIDE = 5; // x, y, vx, vy, life
    ...
    private float blackHoleX, blackHoleY; // 黑洞当前位置
    private List<PointF> writtenPath = new ArrayList<>(); // 已书写的路径点
    ...
    private void initParticles() {
        ...
        for (int i = 0; i < PARTICLE_COUNT; i++) {
            ...
            particleData[i * STRIDE + 2] = 0; // vx
            particleData[i * STRIDE + 3] = 0; // vy
            particleData[i * STRIDE + 4] = 1.0f; // life
        }
    ...
    private void initTextPath() {
        ...
        for (String line : lines) {
            ...
            int steps = (int) (length / 2); // 每2像素一个点
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (deltaTime > 0.1f) deltaTime = 0.016f; // 防止卡顿时跳帧过大
        lastTimeNs = now;
        ...
        switch (currentState) {
            case CONVERGE:
                ...
                if (convergeTimer >= CONVERGE_DURATION || convergeTimer + deltaTime >= CONVERGE_DURATION) {
            ...
            case WRITING:
                ...
                // 记录已走过的路径点
                if (writtenPath.isEmpty() || targetIndex > writtenPath.size()) {
